...
#menyimpan proses database sqlalchemy
...
def predict():
    if request.method == 'POST':
        ...
        # </body>
        # </html>
    # """, road=label[index], prob=round(classes[0][index] * 100, 2), user_image=file_path)
            # response = {
            #     'road': label[index],
            #     'prob': round(classes[0][index]*100,2),
            ...
            return jsonify({'error': 'File not received'})
    else:
        return jsonify({'message': 'Send a POST request to this endpoint with a file'})
        # else:
        #     return render_template('index.html')
